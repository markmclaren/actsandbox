...
6. **Subdirectory Path Mapping & Traversal Protection**: Implemented relative path mapping resolving in [backend/sandbox.py](backend/sandbox.py) via a new `_get_host_path` method. This preserves the nested folder structure (fixing the directory-stripping bug) and prevents directory traversal attacks by validating resolved host paths stay within the workspace boundary.
7. **Non-Blocking Container Command Executions**: Wrapped the blocking `sandbox.execute(...)` calls in [backend/agent.py](backend/agent.py) with `asyncio.to_thread`. This ensures that execution runs in a separate worker thread, leaving the main FastAPI event loop fully free to process incoming WebSocket pings/disconnects and serve concurrent API requests.
8. **Resolved Model Parameter UI Race Condition**: Fixed a subtle race condition in [frontend/app.js](frontend/app.js) where focused model inputs cleared temporarily (for suggestion list display) but failed to restore before launching, leading to `model is required` errors. Added global state restoration in the client and robust backend fallback values in [backend/app.py](backend/app.py) to automatically heal any empty string config fields.
